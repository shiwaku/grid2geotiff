...
## 選定条件 — 正解データと突き合わせられること

検証に使うデータは、次の4つを**すべて**満たすものを選ぶ。

1. **実際に格子に載っている**（`inspect` が通る）
2. **「グリッドデータ」として配布されている**
3. **テキスト（XYZ）で配布されている** …… `convert` の入力になる
4. **同じ格子の GeoTIFF も公開されている** …… 変換結果と突き合わせる**正解データ**になる

4 が肝で、**変換結果をピクセル単位で照合できる**。本ツールの中心的な主張である「セル中心で
記録された座標を半セルずらして配置する」が正しいかどうかは、配布元が作った GeoTIFF と重ねれば
すぐ分かる。ずれていれば値が1セル分まるごと食い違う。

この条件で 18 データセットを絞ると、**山梨県だけが残る**。

| データセット | テキスト | 同じ格子の GeoTIFF | 判定 |
|---|---|---|---|
| **山梨県** | DEM / DSM1 / DSM2 の txt 形式 | **DEM / DSM1 / DSM2 の GeoTIFF 形式** | **○** |
| 神奈川県 | 数値地形図グリッド CSV、DCHM/DSM の TXT | 「メッシュ形式」が GeoTIFF か未確認 | △ |
| 東京都 | なし（グリッドは GeoTIFF のみ） | 0.25m / 0.50m | × テキストが無い |
| 静岡県（7件） | あり（0.5m txt） | なし | × GeoTIFF が無い |
| 熱海（エアロトヨタ） | グリッド TXT 43MB | オルソ写真のみ | × 標高の正解にならない |
| 新潟県村上市 | GRD 形式 | オルソ・陰陽図のみ | × 同上 |

**GeoTIFF があっても中身がオルソ写真や赤色立体地図なら正解データにはならない。** 神奈川・熱海・
村上はこれで落ちる。神奈川のグリッド側は「CSV形式」と「メッシュ形式」の2本だが、カタログの
説明文が両方とも「グラウンドデータを0.5mメッシュに整えたデータです。」で同一のため、メッシュ形式が
GeoTIFF なのかテキストのメッシュなのかを判別できない。

山梨県が条件に合うのは、**同じ図郭の同じ格子を txt と GeoTIFF の両方で出している**ためで、
しかも DEM / DSM1 / DSM2 の3系統すべてが対になっている。

### 突き合わせるときの注意

- **`AREA_OR_POINT` の解釈**が配布元の GeoTIFF と一致しているか。ここが食い違うと原点が半セルずれる
- **NoData の値**。本ツールは `-9999` に統一するが、配布元が別の値や NaN を使っていれば欠損セルを比較できない
